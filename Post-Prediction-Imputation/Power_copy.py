...
def run(Nsize, Single, filepath, adjust, Missing_lambda,strata_size, small_size,verbose=0):
    ...
    p_values, reject, test_time = Framework.retrain_test(Z, X, M, Y,strata_size=strata_size, L=L,G=BayesianRidge,verbose=1)
    ...
    if small_size == False:
        ...
        p_values, reject, test_time = Framework.retrain_test(Z, X, M, Y, strata_size=strata_size,L=L, G=LightGBM, verbose=1)       
        Framework = Retrain.RetrainTest(N = Nsize, covariance_adjustment=0)
        LightGBM = IterativeImputer(estimator=lgb.LGBMRegressor(n_jobs=1,verbosity=-1), max_iter=max_iter) 
        p_values, reject, test_time = Framework.retrain_test(Z, X, M, Y, strata_size=strata_size,L=L, G=LightGBM, verbose=1)
