...
def plot_results(data, title, xsticks):
    columns = ['beta', 'Imputer_Median', 'Imputer_PREP-RidgeReg',  'Imputer_PREP-LightGBM','Imputer_PREP-Xgboost', 'Imputer_Oracle']

    df = pd.DataFrame(data, columns=columns)

    plt.figure(figsize=(10, 6))

    colors = {'Median': 'blue', 'PREP-RidgeReg': 'red', 'PREP-LightGBM': 'green','PREP-Xgboost': 'orange' , 'Oracle':'purple'}
    linestyles = {'Imputer': '-'}
...
def plot(range,range_small, path,path_small, title, title_small, multiple = False):
    ...
    for coef in range_small:
        ...
        for directory in [path_small + "/%f" % (coef)]:
            ...
            row_power_small.extend([results['median_power'], results['lr_power'],results['lightgbm_power'], results['xgboost_power'],results['oracle_power']])
