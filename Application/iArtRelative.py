...
def T2(z, y, y_non_missing):
    ...
    Args:
    - z (np.array): An array indicating group membership.
    - y (np.array): The array of values for which ranks need to be determined.
    - y_non_missing (np.array): The array of non-missing values used to establish ranks.
    
    Returns:
    - t (float): The sum of ranks for the group where z == 1.
    """
    sorted_X = sorted(y_non_missing)
    Y_rank = []

    # Find the closest lower bound in sorted_X for each Y
    for Y in y:
        # Using binary search to find the appropriate index
        index = np.searchsorted(sorted_X, Y, side='right') - 1
        # Adjust index if Y is smaller than all elements in sorted_X
        if index == -1:
            index = 0
        # Append the rank (index + 1 because ranks start from 1)
        Y_rank.append(index + 1)
