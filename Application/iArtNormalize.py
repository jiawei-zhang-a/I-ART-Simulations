...
def getnormalizedY(G, Z, X, Y, covariate_adjustment, mode, S):
    ...
    if mode == 'cluster':
        ...
        # Adjust Y by cluster sizes for normalization
        Y_normalized = np.array([y / cluster_size_map[s] for y, s in zip(Y, S.flatten())])
...
def test(*,Z, X, Y, G='bayesianridge', S=None,L = 10000,threshholdForX = 0.2, mode = 'strata',verbose = False, covariate_adjustment = 0, random_state=None, alternative = "greater", alpha = 0.05):
    ...
    Y_pred = getnormalizedY(clone(G_model), Z, X, Y, covariate_adjustment, mode, S)
    ...
    for l in range(L):
        ...
        Y_pred = getnormalizedY(clone(G_model), Z_sim, X, Y, covariate_adjustment, mode, S)
