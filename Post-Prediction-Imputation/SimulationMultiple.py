...
class DataGenerator:
  ...
  def GenerateY(self, X, U, Z,  StrataEps, IndividualEps):
    ...
    Z = Z.reshape(-1,)
      ...
      # Calculate Y_n1
    Y_n1 = 1/4 * self.beta * Z  + sum2 + sum3 + StrataEps[:,0]  + IndividualEps[:,0]

    # Compute Yn2
    Y_n2 = self.beta * Z  + self.beta * Z * X[:,0] + self.beta * Z * U + sum3 + sum4  + StrataEps[:,1] + IndividualEps[:,1]

    # Compute Yn3
    Y_n3 = self.beta * Z * sum5  + sum9 + U +  StrataEps[:,2]  + IndividualEps[:,2]
