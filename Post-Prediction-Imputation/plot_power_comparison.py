...
def plot_results(data, title):
    columns = ['beta', 'Size_Median', 'Size_Linear', 'Size_XGBoost', 'Size_LightGBM']

    df = pd.DataFrame(data, columns=columns)

    plt.figure(figsize=(10, 6))

    colors = {'Median': 'blue', 'Linear': 'red', 'XGBoost': 'green', 'LightGBM': 'orange' }
    linestyles = {'Size': '-'}
...
def plot(range,dir,title):
    ...
    for coef in range:
        ...
        for directory in ["Result/%s/%f" % (dir,coef)]:
            ...
            row_power.extend([results['median_power'], results['lr_power'], results['xgboost_power'],results['lightGBM_power']])
...
def main():
    ...
    plot(np.arange(0.0,0.05,0.05),"HPC_power_2000_unobserved_nonlinearZ_nonlinearX" + "_single","Size-1000, nonlinearZ_nonlinearX, U")
