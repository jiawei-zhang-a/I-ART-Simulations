...
class DataGenerator:
  def __init__(self, N, N_T, beta_11, beta_12, beta_21, beta_22, beta_23, beta_31):
    self.N = N
    self.N_T = N_T
  ...
  def GenerataY(self, X, U, Z):
    ...
    # Calculate Y_n1
    Y_n1 = (self.beta_11 * Z + self.beta_12 * Z * sum1   + sum2 + np.sin(U[:,0]) + U[:,1]) 

    # Compute Yn2
    Y_n2 = (self.beta_21 * Z + self.beta_22 * Z * X[:,0] + self.beta_23 * Z * U[:,0] * U[:,1] + sum3 + sum4) 

    # Compute Yn3
    Y_n3 = (self.beta_31 * Z + sum5 + sum6 + X[:,0] * X[:,1] * np.sin(U[:,0] * U[:,1])) 
  ...
  def GenerateM(self, X, U, Y, lambda1, lambda2, lambda3, single = True):
      n = X.shape[0]
      M = np.zeros((n, 3))
...
def GenerateData(self):  
  ...
  # Generate M
  M = self.GenerateM(X, U, Y, self.lambda1, self.lambda2, self.lambda3)
