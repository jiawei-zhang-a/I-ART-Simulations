...
class RetrainTest:
    ...
    def sort(self, Z, Y, S, M):
        ...
        if len(Z.shape) == 1 or Z.shape[1] != 1:
            Z = Z.reshape(-1, 1)
        if len(Y.shape) == 1 or Y.shape[1] != Y.shape[1]:
    ...
    def check_param(self,Z, Y, S, M, G, L, verbose, alpha):
        ...
        if not np.all(np.isin(Z, [0, 1, 0.0, 1.0])):
            raise ValueError("Z must contain only 0, 1, 0.0, or 1.0")
    ...
    def retrain_test(self,*,Z, Y, S,G, M = None, L = 10000,verbose = False, alpha = 0.05):
        """
        ...
        #print train end
        if verbose:
            print("Observed Wilconxin rank-sum test result:"+str(t_obs))
        ...
        for l in range(L):
            ...
            if verbose:
                print("Completed "+str(l+1)+" simulations out of "+str(L))
                print("Monte Carlo Wilconxin rank-sum test result:["+str(l)+"]:"+str(t_sim[l]))

        if verbose:
            print("Monte Carlo t_mean:"+str(np.mean(t_sim)))
            print("\n")
        ...
        if verbose:
            print("the time used for the retrain framework:"+str(time.time() - start_time) + " seconds")
