...
def split(y, z, M):
    ...
    y_missing = y[missing_indices].reshape(-1,1)
    y_non_missing = y[non_missing_indices].reshape(-1,1)

    z_missing = z[missing_indices].reshape(-1,1)
    z_non_missing = z[non_missing_indices].reshape(-1,1)
...
def getT(y, z, lenY, M):
    ...
    for i in range(lenY):
        ...
        # Fit a Bayesian Ridge Regression model on non-missing data
        lr = lgb.LGBMRegressor()
        lr.fit(z_non_missing, y_non_missing)  # Assuming z are the features and y is the target

        # Predict the non-missing targets to calculate residuals
        y_pred = lr.predict(z_non_missing)
        residuals = y_non_missing - y_pred

        # Calculate the variance of these residuals
        var_residue = np.var(residuals)

        y_missing = y_missing.reshape(-1,)
        y_non_missing = y_non_missing.reshape(-1,)

        y_missing = y_missing + np.random.normal(0, np.sqrt(var_residue), len(y_missing))
        ...
        y_combined = np.concatenate((y_missing, y_non_missing))
        z_combined = np.concatenate((z_missing, z_non_missing))
